...
public interface VueAllEvpRepository extends EntityRepository<VueAllEvp, Integer> {
    ...
    @Query("SELECT sum(v.totalEvp) FROM VueAllEvp v WHERE v.departEff like :annee and v.trafic like :trafic and v.source =:source")
    ...
    @Query("SELECT v FROM VueSourceEvp v WHERE v.annee =:annee")
    List<VueSourceEvp> listeEVPParSourceEtParMois(@QueryParam("annee") String mois);
    ...
    @Query("SELECT sum(v.totalEvp) FROM VueAllEvp v WHERE v.departEff like :annee and v.source =:source")
    BigInteger totalEVPParSourceEtParAn(@QueryParam("annee") String annee, @QueryParam("source") String source);
    @Query("SELECT sum(v.plein) FROM VueAllEvp v WHERE v.departEff like :annee and v.source =:source")
    BigInteger totalEVPPleinParSourceEtParAn(@QueryParam("annee") String annee, @QueryParam("source") String source);
    @Query("SELECT sum(v.vide) FROM VueAllEvp v WHERE v.departEff like :annee and v.source =:source")
    BigInteger totalEVPVideParSourceEtParAn(@QueryParam("annee") String annee, @QueryParam("source") String source);
