...
public interface TdrCongoTerminalRepository extends EntityRepository<TdrCongoTerminal,Integer> {
    ...
    @Query("SELECT sum(t.evp) FROM TdrCongoTerminal t WHERE  t.trafic = 'TRANSIT' and t.annee = :annee")
    ...
    @Query("SELECT sum(t.poids) FROM TdrCongoTerminal t WHERE  t.trafic = 'TRANSIT' and t.annee = :annee")
